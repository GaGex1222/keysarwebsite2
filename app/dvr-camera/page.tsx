...
export default function RecordingSystemsPage() {
  ...
  return (
    <div className="min-h-screen bg-sky-50 text-slate-900 font-sans selection:bg-sky-200/30" dir="rtl">
      ...
      {/* Hero Section */}
      <section className="relative pt-32 pb-20 px-6 border-b border-sky-100 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          >
            <div className="flex items-center gap-3 text-sky-600 mb-6 font-black italic tracking-widest text-sm">
            ...
            <h1 className="text-5xl md:text-8xl font-[1000] italic leading-[0.85] tracking-tighter uppercase mb-8">
              מערכות הקלטה <br/>
              <span className="text-sky-600">DVR & NVR</span>
            </h1>
            <p className="text-xl text-slate-600 max-w-3xl leading-relaxed italic border-r-4 border-sky-500 pr-6">
      ...
      <main className="max-w-7xl mx-auto px-6 py-20">
        ...
        <section className="mb-32">
          <div className="flex items-center justify-between mb-12 flex-wrap gap-4">
            ...
            <div className="text-xs font-mono text-slate-500 bg-sky-50 px-4 py-2 rounded-full border border-sky-200">
              Last_Update: Sept_2021 | Data_Ref: 2023_Call_Support
            </div>
          </div>
          
          <div className="overflow-x-auto rounded-[2rem] border border-sky-200 bg-sky-100 backdrop-blur-md">
            <table className="w-full text-right border-collapse">
              <thead>
                <tr className="bg-sky-50 text-sky-600 uppercase text-xs font-black italic tracking-widest">
                  <th className="p-6 border-b border-sky-200">שם המכשיר</th>
                  <th className="p-6 border-b border-sky-200">יצרן</th>
                  <th className="p-6 border-b border-sky-200">תכונות מרכזיות</th>
                  <th className="p-6 border-b border-sky-200 text-left">מחיר משוער*</th>
                </tr>
              </thead>
              <tbody className="text-slate-700">
                {dvrModels.map((m, i) => (
                  <tr key={i} className="hover:bg-sky-50 transition-colors border-b border-sky-100 last:border-0 group">
                    <td className="p-6 font-black text-slate-900 group-hover:text-sky-600 transition-colors">{m.name}</td>
                    <td className="p-6 italic">{m.brand}</td>
                    <td className="p-6 text-sm italic">{m.features}</td>
                    <td className="p-6 text-left font-mono font-bold text-sky-600">{m.price}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="mb-32 grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div className="bg-white p-10 rounded-[2.5rem] border border-sky-200 relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity">
              <HardDrive size={120} />
            </div>
            <h3 className="text-3xl font-[1000] italic mb-6 text-slate-900 uppercase">מה זה DVR?</h3>
            ...
            <div className="flex items-center gap-4 text-sky-600 font-black italic text-sm">
              <Zap size={18} /> LOCAL_STORAGE_CORE
            </div>
          </div>

          <div className="bg-sky-600 p-10 rounded-[2.5rem] text-white relative overflow-hidden group">
        ...
        <section className="mb-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              ...
              <div key={i} className="flex gap-6 p-8 bg-sky-50 rounded-3xl border border-sky-100 hover:border-sky-500/30 transition-all">
                <div className="text-sky-600 text-2xl font-black italic">0{i+1}</div>
                <div>
                  <h4 className="text-xl font-black italic mb-2 text-slate-900">{diff.t}</h4>
                  <p className="text-slate-600 italic text-sm">{diff.d}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Use Cases & Analytics */}
        <section className="mb-32 py-20 bg-sky-50 rounded-[3rem] border border-sky-500/10 px-10">
          <h3 className="text-3xl font-[1000] italic mb-12 text-center uppercase">שימושי מערכות הקלטה</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            <div className="space-y-6">
              <div className="flex items-center gap-3 text-sky-600 font-black italic uppercase text-xs tracking-widest"><BarChart size={16}/> Business_Protection</div>
              <ul className="space-y-4 text-slate-700 italic text-sm">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3 text-sky-600 font-black italic uppercase text-xs tracking-widest"><Users size={16}/> Human_Monitoring</div>
              <ul className="space-y-4 text-slate-700 italic text-sm">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3 text-sky-600 font-black italic uppercase text-xs tracking-widest"><Cpu size={16}/> Advance_Analytics</div>
              <ul className="space-y-4 text-slate-700 italic text-sm">
        ...
        <section className="mb-20">
          <h2 className="text-4xl font-[1000] italic mb-16 uppercase border-r-4 border-sky-500 pr-6">מותגים מומלצים</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="p-10 bg-white border border-sky-100 rounded-[2.5rem] group hover:border-sky-500/50 transition-all">
                <h4 className="text-2xl font-black italic mb-2 text-slate-900">{brand.name}</h4>
                <div className="text-sky-600 text-xs font-black mb-6 italic uppercase tracking-widest">{brand.origin}</div>
                <p className="text-slate-600 italic text-sm leading-relaxed">{brand.desc}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      {/* Footer / CTA */}
      <footer className="py-20 text-center bg-sky-100 border-t border-sky-100 relative z-20">
        <div className="max-w-2xl mx-auto px-6">
          <h3 className="text-3xl font-[1000] italic mb-6 uppercase">מחפש מערכת אבטחה איכותית?</h3>
          <p className="text-slate-600 italic mb-10">הגעת למקום הנכון. צוות קיסר זמין עבורך לייעוץ והתאמת המערכת המדויקת ביותר.</p>
