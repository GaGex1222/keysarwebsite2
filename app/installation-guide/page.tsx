...
export default function SecurityInstallationGuide() {
  ...
  return (
    <div className="min-h-screen bg-sky-50 text-slate-900 font-sans selection:bg-sky-200 selection:text-black" dir="rtl">
      ...
      <section className="relative pt-32 pb-20 px-6 max-w-7xl mx-auto text-center">
        ...
        <p className="text-xl md:text-2xl text-slate-600 max-w-3xl mx-auto italic font-light leading-relaxed">
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 relative">
        <div className="bg-sky-50 border border-sky-200 rounded-[3rem] overflow-hidden backdrop-blur-xl">
          <div className="p-10 border-b border-sky-200 flex flex-col md:flex-row justify-between items-center gap-6">
            <h2 className="text-3xl font-[1000] italic uppercase">מחירון מצלמות אבטחה 2026</h2>
            <div className="text-xs text-sky-600 font-black italic tracking-widest bg-sky-100 px-4 py-2 rounded-lg border border-sky-500/20">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-right border-collapse">
              <thead>
                <tr className="bg-sky-50 text-slate-500 text-xs uppercase tracking-widest italic font-black">
                ...
                {cameraPrices.map((item, i) => (
                  <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                    <td className="p-8 flex items-center gap-4">
                      <span className="text-sky-600 group-hover:scale-110 transition-transform">{item.icon}</span>
                      <span className="font-black italic text-lg">{item.type}</span>
                    </td>
                    <td className="p-8 font-mono text-xl text-sky-600">{item.price}</td>
                    <td className="p-8 hidden md:table-cell">
                      <div className="flex justify-center">
                        <div className="w-24 h-1.5 bg-sky-100 rounded-full overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          {[
            ...
            <div key={i} className="relative group">
              <div className="absolute -top-4 -right-4 text-6xl font-[1000] text-slate-900/[0.03] group-hover:text-sky-600/10 transition-colors">
                {spot.id}
              </div>
              <div className="p-8 bg-sky-50 border border-sky-200 rounded-3xl h-full flex flex-col items-center justify-center text-center hover:border-sky-500/50 transition-all">
                <div className="text-sky-600 mb-6 group-hover:scale-125 transition-transform">{spot.icon}</div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col lg:flex-row gap-20">
            <div className="lg:w-1/3 space-y-10">
              <h2 className="text-5xl font-[1000] italic leading-none uppercase pr-6 border-r-8 border-sky-500">מדריך <br/>ההתקנה <br/>של קיסר</h2>
              ...
              <div className="p-6 bg-sky-50 rounded-2xl border border-sky-200">
                 <div className="flex gap-4 items-center mb-4 text-sky-600 font-black italic">
                   <MonitorPlay size={20} />
            ...
            <div className="lg:w-2/3 grid grid-cols-1 md:grid-cols-3 gap-6">
              {installationSteps.map((step, i) => (
                <div key={i} className="p-10 bg-sky-50 border border-sky-200 rounded-[3rem] relative group hover:bg-black transition-colors">
                  <div className="text-4xl font-[1000] text-sky-600/20 mb-6 group-hover:text-sky-600 transition-colors italic">0{i+1}</div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-32">
        <div className="bg-white p-12 rounded-[4rem] border border-sky-200 overflow-hidden relative">
          <div className="absolute top-0 right-0 w-64 h-64 bg-sky-50 blur-[100px]" />
      ...
      {/* FAQ / Q&A Section */}
      <section className="max-w-4xl mx-auto px-6 py-20 border-t border-sky-100">
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="p-8 bg-sky-50 rounded-[2rem] border border-sky-200 group hover:border-sky-500 transition-all">
              <div className="flex gap-4 items-start mb-4">
                 <CheckCircle className="text-sky-600 shrink-0 mt-1" size={18} />
                 <h4 className="text-xl font-black italic">{faq.q}</h4>
              </div>
              <p className="text-slate-600 text-sm italic pr-8">{faq.a}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-32 relative text-center">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10">
          <h2 className="text-6xl md:text-8xl font-[1000] italic mb-10 leading-none uppercase italic">
            READY TO <br/>
            <span className="text-sky-600">UPGRADE?</span>
          </h2>
          <p className="text-xl text-slate-600 italic mb-12 font-light">
            קיסר מערכות אבטחה – השותף שלכם לשקט נפשי. תכנון, התקנה ושירות מקצועי ללא פשרות.
          </p>
          <div className="flex flex-col md:flex-row gap-6 justify-center">
            <button className="bg-sky-600 text-white px-12 py-5 rounded-2xl font-[1000] text-xl italic hover:shadow-[0_0_20px_rgba(2,132,199,0.5)] transition-all">
              תיאום טכנאי קיסר
            </button>
