...
export default function BusinessSecurityPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-sky-200/30" dir="rtl">
      
      {/* Hero Section */}
      <section className="relative pt-32 pb-20 px-6 border-b border-sky-100">
        <div className="max-w-7xl mx-auto">
          <motion.div 
            ...
            className="max-w-4xl"
          >
            <div className="flex items-center gap-3 text-sky-600 mb-6 font-black italic uppercase tracking-widest text-sm">
              <Shield size={20} /> Professional_Security_Protocol
            </div>
            <h1 className="text-5xl md:text-7xl font-[1000] italic leading-tight mb-8">
              מצלמות אבטחה <br/> 
              <span className="text-sky-600">לניהול ומיגון העסק</span>
            </h1>
            <p className="text-xl text-slate-600 leading-relaxed max-w-2xl border-r-2 border-sky-200 pr-6 italic">
      ...
      <main className="max-w-7xl mx-auto px-6 py-20">
        ...
        <div className="bg-white/30 border border-sky-100 p-8 rounded-3xl mb-20 backdrop-blur-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              <div key={i} className="flex items-center gap-2 text-slate-600 hover:text-sky-600 transition-colors cursor-pointer group">
        ...
        <section className="mb-32">
          ...
          <div className="overflow-x-auto rounded-3xl border border-sky-200 bg-sky-100">
            <table className="w-full text-right border-collapse">
              <thead>
                <tr className="bg-sky-50 text-sky-600 uppercase text-xs font-black italic tracking-widest">
                  <th className="p-6 border-b border-sky-200">סוג מצלמה</th>
                  <th className="p-6 border-b border-sky-200">תיאור</th>
                  <th className="p-6 border-b border-sky-200">טווח מחיר</th>
                  <th className="p-6 border-b border-sky-200">פירוט פעולה</th>
                </tr>
              </thead>
              <tbody className="text-slate-700">
                {cameraTypes.map((cam, i) => (
                  <tr key={i} className="hover:bg-sky-50 transition-colors group">
                    <td className="p-6 font-black text-slate-900">{cam.type}</td>
                    <td className="p-6 italic">{cam.desc}</td>
                    <td className="p-6 text-sky-600 font-mono font-bold">{cam.price}</td>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-32">
          <div className="lg:col-span-2 space-y-12">
            <div className="bg-white/20 p-10 rounded-[2.5rem] border border-sky-100 relative overflow-hidden group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {[
                ...
                <div key={i} className="p-8 bg-sky-50 rounded-3xl border border-sky-100">
                  <div className="text-sky-600 mb-4">{benefit.icon}</div>
                  <h4 className="text-xl font-black italic mb-3">{benefit.title}</h4>
                  <p className="text-slate-600 leading-relaxed text-sm italic">{benefit.text}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-sky-600 text-white p-10 rounded-[2.5rem] flex flex-col justify-between">
            <div>
              ...
              <ul className="space-y-6">
                {[
                  ...
                  <li key={i} className="flex items-start gap-3 font-bold italic border-b border-black/10 pb-2">
                    <span className="text-xs bg-black text-slate-900 w-5 h-5 flex items-center justify-center rounded-full shrink-0">{i+1}</span>
                    {item}
                  </li>
                ))}
              </ul>
            </div>
            <div className="mt-12 bg-black text-slate-900 p-6 rounded-2xl">
              <p className="text-xs font-black uppercase tracking-widest text-sky-600 mb-2">Pro Tip:</p>
        ...
        <section className="mb-32">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <Radio className="mx-auto text-sky-600 mb-4" size={40} />
            <h2 className="text-4xl font-[1000] italic uppercase mb-6 text-slate-900">קווית או אלחוטית?</h2>
            <p className="text-slate-600 italic">הדילמה הטכנולוגית של כל בעל עסק. הנה מה שחשוב לדעת לפני שמחליטים:</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="border border-sky-200 p-10 rounded-[2.5rem] bg-gradient-to-br from-white/5 to-transparent">
              <h4 className="text-2xl font-black italic mb-6 text-sky-600">אלחוטית (Wi-Fi)</h4>
              <ul className="space-y-4 text-slate-700 italic">
            ...
            <div className="border border-sky-200 p-10 rounded-[2.5rem] bg-gradient-to-br from-cyan-500/10 to-transparent">
              <h4 className="text-2xl font-black italic mb-6 text-slate-900">קווית (Cabled)</h4>
              <ul className="space-y-4 text-slate-700 italic">
        ...
        <section className="max-w-4xl mx-auto mb-20">
          ...
          <div className="space-y-6">
            {[
              ...
              <div key={i} className="bg-sky-50 border border-sky-200 p-8 rounded-3xl">
                <h5 className="text-xl font-black italic text-sky-600 mb-4">{faq.q}</h5>
      ...
      <footer className="py-20 text-center border-t border-sky-100 opacity-50">
        <div className="flex justify-center gap-8 mb-8">
           <Shield className="text-sky-600" />
           <Video className="text-sky-600" />
           <hardDrive className="text-sky-600" />
        </div>
