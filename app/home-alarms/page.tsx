      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {mainComponents.map((comp, i) => (
              <motion.div 
                ...
                className="p-8 bg-white/5 border border-white/10 rounded-[2.5rem] transition-all group"
              >
        ...
        <section className="mb-32 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="relative group">
             ...
             <div className="relative bg-white/5 border border-white/10 rounded-[4rem] p-12 overflow-hidden">
                ...
                <ul className="space-y-4">
                  {["דריכה ונטרול מרחוק", "צפייה במצלמות בזמן אמת", "דיווח על חזרת ילדים מהביה\"ס", "ניטור טמפרטורה וצריכת חשמל"].map((item, i) => (
                    <li key={i} className="flex items-center gap-3 text-sm font-bold italic text-slate-300">
          ...
          <div className="space-y-8">
             ...
             <div className="p-8 bg-white/5 border border-white/10 rounded-[3rem]">
                ...
                <div className="grid grid-cols-2 gap-4">
                   ...
                   <div className="flex items-center gap-3 text-xs font-black italic"><Wind size={16} className="text-slate-400"/> דליפת גז</div>
                   <div className="flex items-center gap-3 text-xs font-black italic"><Droplets size={16} className="text-cyan-500"/> הצפת מים</div>
        ...
        <section className="mb-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {monitoringMethods.map((method, i) => (
              <div key={i} className={`p-10 rounded-[3rem] border transition-all ${method.active ? 'bg-blue-600 text-black border-blue-500' : 'bg-white/5 border-white/10 text-white'}`}>
                <div className="mb-6">{method.icon}</div>
                <h3 className="text-2xl font-[1000] italic mb-2 uppercase">{method.type}</h3>
                <div className="flex gap-2 mb-6">
                  <span className={`text-[10px] font-black px-2 py-0.5 rounded border ${method.active ? 'border-black/20' : 'border-white/10'}`}>מהירות: {method.speed}</span>
                </div>
        ...
        <section className="mb-32 grid grid-cols-1 lg:grid-cols-2 gap-8">
           <div className="p-12 bg-white/5 border border-white/10 rounded-[4rem]">
              ...
              <p className="text-slate-400 italic mb-6 text-sm leading-relaxed">
