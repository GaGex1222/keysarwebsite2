  ...
  return (
    <div className="min-h-screen bg-[#0f1115] text-white font-sans selection:bg-cyan-500/30" dir="rtl">
      ...
      <section className="relative pt-32 pb-24 px-6 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          >
            ...
            <p className="text-xl md:text-2xl text-slate-400 font-light leading-relaxed mb-10 border-r-4 border-cyan-500 pr-6 italic">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            <motion.div 
              key={i}
              whileHover={{ y: -10 }}
              className="p-8 bg-[#1c1f26] border border-white/5 rounded-[2rem] hover:border-cyan-500/30 transition-all group"
            >
              <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-cyan-500 mb-6 group-hover:bg-cyan-500 group-hover:text-black transition-all">
                {item.icon}
              </div>
              <h3 className="text-xl font-black italic mb-3">{item.title}</h3>
              <p className="text-slate-400 text-sm italic leading-relaxed">{item.desc}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="py-32 bg-white/5 relative">
        <div className="max-w-7xl mx-auto px-6">
          <h2 className="text-4xl md:text-6xl font-[1000] italic text-center mb-20 uppercase tracking-tighter">
            למי זה <span className="text-cyan-500">רלוונטי?</span>
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {useCases.map((use, i) => (
              <div key={i} className="text-center group">
                <div className="inline-flex p-6 rounded-full bg-black border border-cyan-500/20 text-cyan-500 mb-6 group-hover:scale-110 transition-transform shadow-[0_0_30px_rgba(6,182,212,0.1)]">
                  {use.icon}
                </div>
                <h4 className="text-2xl font-black italic mb-4">{use.title}</h4>
                <p className="text-slate-400 italic leading-relaxed">{use.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-32">
        <div className="flex flex-col lg:flex-row gap-20 items-center">
          <div className="w-full lg:w-1/2">
            <h2 className="text-4xl font-[1000] italic mb-8 border-r-4 border-cyan-500 pr-6 uppercase">Technology_Core</h2>
            <p className="text-slate-300 text-lg mb-12 italic leading-relaxed">
            ...
            <div className="space-y-8">
              {[
                ...
                <div key={i} className="flex gap-4">
                  <div className="mt-1 text-cyan-500"><Info size={20} /></div>
                  <div>
                    <h5 className="font-black italic text-white">{point.label}</h5>
                    <p className="text-sm text-slate-400 italic">{point.detail}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="w-full lg:w-1/2 relative">
             <div className="aspect-square bg-gradient-to-tr from-cyan-500/20 to-purple-500/10 rounded-[3rem] border border-white/10 flex items-center justify-center relative overflow-hidden group">
                {/* Visual Placeholder for a Phone/App mockup */}
                <Smartphone size={150} className="text-white/20 group-hover:scale-110 group-hover:text-cyan-500 transition-all duration-700" strokeWidth={1} />
                <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-20" />
                <div className="absolute bottom-10 inset-x-10 p-6 bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl">
      ...
      <section className="max-w-5xl mx-auto px-6 py-20 mb-20">
        <div className="bg-[#1c1f26] rounded-[2.5rem] border border-white/5 overflow-hidden shadow-2xl">
          ...
          <div className="divide-y divide-white/5">
            {faqItems.map((faq, i) => (
              <div key={i} className="p-8 flex flex-col md:flex-row gap-6 hover:bg-white/5 transition-colors">
                <div className="md:w-1/3 font-black italic text-cyan-500 text-lg">
                   {faq.q}
                </div>
                <div className="md:w-2/3 text-slate-400 italic leading-relaxed">
                   {faq.a}
                </div>
              </div>
            ))}
      ...
      <section className="py-32 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6">
          ...
          <motion.button 
            ...
            whileTap={{ scale: 0.95 }}
            className="bg-cyan-500 text-black px-12 py-6 rounded-2xl font-[1000] text-2xl italic shadow-[0_10px_40px_rgba(6,182,212,0.4)]"
          >
            תיאום ייעוץ מקצועי
          </motion.button>
        </div>
